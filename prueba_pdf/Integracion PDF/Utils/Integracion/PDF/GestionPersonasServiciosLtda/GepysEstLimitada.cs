...
{
    public class GepysEstLimitada
    {
        #region Variables
        private readonly Dictionary<int, string> _itemsPatterns = new Dictionary<int, string>
        {
            {0, @"^\d{1,}\s\d{1,}\s[a-zA-Z]{1,2}\d{5,6}\s"},
            {1, @"^\d{1,}\s\d{1,}\s[a-zA-Z]{1,2}\s" },
            {2, @"\s[a-zA-Z]{1,2}\d{5,6}$"}
        };
        private const string RutPattern = "Autor:";
        private const string OrdenCompraPattern = "Orden de Compra";
        private const string ItemsHeaderPattern =
            "# Descripción Precio Total";

        //private const string ItemsFootPattern =
        //    "";
        ...
        public GepysEstLimitada(PDFReader pdfReader)
        {
                ...
                switch (optItem)
                {
                    ...
                    case 2:
                        ...
                        var item2 = new Item
                        {
                            ...
                            Precio = (int.Parse(test2[test2.Length - 1].Replace(".", "")) / int.Parse(test2[1])).ToString()
                        };
                        items.Add(item2);
                        break;
                }
