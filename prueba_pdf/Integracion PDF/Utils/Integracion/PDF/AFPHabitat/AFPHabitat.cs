...
namespace IntegracionPDF.Integracion_PDF.Utils.Integracion.PDF.AFPHabitat
{
    class AFPHabitat
    {
        #region Variables
        ...
        private const string RutPattern = "Rut N°";
        private const string OrdenCompraPattern = "Orden de Compra Fecha";
        private const string ItemsHeaderPattern =
            "Lín-Env Descripción Artículo Cantidad";
        ...
        public AFPHabitat(PDFReader pdfReader)
        {
        ...
        public OrdenCompra.OrdenCompra GetOrdenCompra()
        {
            ...
            for (var i = 0; i < _pdfLines.Length; i++)
            {
                ...
                if (!_readRut)
                {
                    ...
                    {
                        OrdenCompra.Rut = "98000100";// GetRut(_pdfLines[i]);
                        _readRut = true;
                    }
                }
        ...
        {
            //HABIT-0000013966
            //Console.Write($"OC: {str}");
            var index = Regex.Match(str, @"\s[a-zA-Z]{5}-\d{10}\s").Index;
            var length = Regex.Match(str, @"\s[a-zA-Z]{5}-\d{10}\s").Length;
