...
namespace IntegracionPDF.Integracion_PDF.Utils.Integracion.PDF.EastonDesign
{
    class EastonDesign
    {
        #region Variables
        ...
        {
            {0, @"^\d{1,}\s\d{1,}-\d{1,}\s\d{1,}"},
            //{1, @"^\d{1,}\s\w{3}\d{5,6}\s\d{1,}\s" }
        };
        private const string RutPattern = "Fábrica de Muebles RUT :";
        private const string OrdenCompraPattern = "Nº";
        private const string ItemsHeaderPattern =
            "Item SM - NP Cant. U.M. Descripción Código Precio % Dcto Total";
        ...
        public EastonDesign(PDFReader pdfReader)
        {
