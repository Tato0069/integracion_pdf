...
namespace IntegracionPDF.Integracion_PDF.Utils.Integracion.PDF.Megasalud
{
            ...
            //foreach(var str in pdfLines)
            {
                ...
                switch (optItem)
                {
                    case 0:
                        ...
                        var item0 = new Item
                        {
                            Cantidad = test0[2].Replace(".", ""),
                            Precio = test1.Length == 1 ? test0[test0.Length - 3] : test1[0],
                            Sku = "W102030"
                        };
                        for (var j = i; j < i + 4; j++)
                        {
                            if (Regex.Match(pdfLines[j], @"^[a-zA-Z]{1,2}\d{5,6}").Success)
                            {
                                item0.Sku = pdfLines[j].Split(' ')[0];
                            }
                        }
                        items.Add(item0);
                        break;
