...
namespace IntegracionPDF.Integracion_PDF.Utils.Integracion.PDF.Rimasa
{
    class Rimasa
    {
        #region Variables
        ...
        public Rimasa(PDFReader pdfReader)
        {
                ...
                switch (optItem)
                {
                    case 0:
                        ...
                        for (; j > 0; j--)
                        {
                            ...
                            .DeleteContoniousWhiteSpace(), @"^\d{1,}\s\$\s\d{1,}$").Success)
                            {
                                var split = pdfLines[j].Trim().DeleteContoniousWhiteSpace().Split(' ');
                                item0.Cantidad = split[0];
                                item0.Precio = split[2];
                                break;
                            }
                        }
                        items.Add(item0);
        ...
        private string GetSku(string[] test1)
        {
            ...
            else
            {
                ...
                {
                    var index = Regex.Match(str, @"\s[a-zA-Z]{2}\s?\d{5}").Index;
                    var length = Regex.Match(str, @"\s[a-zA-Z]{2}\s?\d{5}").Length;
                    ret = str.Substring(index, length).Trim();
                }
            }
            return ret.Replace(" ","");
