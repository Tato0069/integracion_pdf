...
namespace IntegracionPDF.Integracion_PDF.Utils.Integracion.PDF.Candem
{
    public class Candem
    {
        #region Variables
        ...
        private readonly Dictionary<int, string> ItemsPatterns = new Dictionary<int, string>
        {
            ...
            {1, @"^\d{1,}\s\w{3}\d{5,6}\s\d{3,8}\s\d{1,}\s\w{2}\s" }
            //{1, @"^\d{1,}\s\w{3}\d{5,6}\s\d{1,}\s" }
        };
        private const string RutPattern = "CAMDEN SERVICIOS SPA";
