...
namespace IntegracionPDF.Integracion_PDF.Utils.Integracion.PDF.JoyGlobal
{
    class JoyGlobal
    {
        #region Variables
        ...
        {
            {0, @"^\d{1,}\s\d{9}\s" }
            // {0, @"^\d{1,}\s\d{9}\s\d{1,}\s[a-zA-Z]{2,3}\s"},
            //{1, @"^\d{1,}\s\w{3}\d{5,6}\s\d{1,}\s" }
        };
        private const string RutPattern = "Joy Global (Chile) S.A.";
        private const string OrdenCompraPattern = "Joy Global (Chile) S.A. No. Orden";
        private const string ItemsHeaderPattern =
            "UM Total Neto";

        private const string CentroCostoPattern = "";
        private const string ObservacionesPattern = "Enviar factura a Encabezado";
        ...
        public JoyGlobal(PDFReader pdfReader)
        {
                ...
                if (!_readObs)
                {
                    if (IsObservacionPattern(_pdfLines[i]))
                    {
                        OrdenCompra.Observaciones =
                            ...
                            $"{_pdfLines[i+3].Trim().DeleteContoniousWhiteSpace()}";
                        _readObs = true;
                        _readItem = false;
                    }
                }
            ...
            //foreach(var str in pdfLines)
            {
                ...
                switch (optItem)
                {
                    case 0:
                        ...
                        {
                           // Sku = "W102030",
                            Cantidad = test0[2].Split(',')[0],//test1[1], // .Split(',')[0]
                            Precio = test0[4].Replace(".",""),//test1[1].Replace(".",""), // .Replace(".","")
        ...
        {
            var ret = -1;
             //str = str.DeleteContoniousWhiteSpace().Trim();
