...
namespace IntegracionPDF.Integracion_PDF.Utils.Integracion.PDF.AsesoriasServiciosCapacitacionIcyde
{
    class AsesoriasServiciosCapacitacionIcyde
    {
        #region Variables
        private readonly Dictionary<int, string> _itemsPatterns = new Dictionary<int, string>
        {
            {0, @"^\d{1,}\s[0-9a-zA-Z]{1,}"}
            //20 PAPEL FOTOCOPIA CARTA 500 HJS DIMERC 1993 39860
        };
        private const string RutPattern = "DATOS DE FACTURACION";
        private const string OrdenCompraPattern = "Nº ";
        private const string ItemsHeaderPattern = "CANT. DETALLE V. UNITARIO TOTAL";
        ...
        public AsesoriasServiciosCapacitacionIcyde(PDFReader pdfReader)
        {
            ...
            //foreach(var str in pdfLines)
            {
                ...
                switch (optItem)
                {
                    case 0:
                        ...
                        {
                            Sku = "W102030",
                            Descripcion = test0.ArrayToString(1,test0.Length - 2),
                            Cantidad = test0[0],
                            Precio = test0[test0.Length - 2].Split(',')[0]
                        };
                        if (item0.Cantidad.Equals("0")) break;
                        item0.Descripcion = item0.Descripcion.Replace("HJS","HJ");
                        items.Add(item0);
                        break;
                }
            }
