...
namespace IntegracionPDF.Integracion_PDF.Utils.Integracion.PDF.UNAB
{
    public class CorpUnab
    {
        ...
        public CorpUnab(PDFReader pdfReader)
        {
        ...
        private List<Item> GetItems(string[] pdfLines, int firstIndex)
        {
            ...
            for (; firstIndex < pdfLines.Length; firstIndex++)
            {
                ...
                else if (Regex.Match(str, @"^\d{1,}-\s\d{1,}\s\w{1}\d{6}\s").Success)
                {
                    var test = pdfLines[firstIndex].Trim().DeleteContoniousWhiteSpace().Split(' ');
                    var item = new Item
                    {
                        Sku = test[2],
                        Cantidad = test[test.Length - 4].Split('.')[0].Replace(",", "."),
        ...
        public OrdenCompra.OrdenCompra GetOrdenCompra()
        {
            ...
            for (var i = 0; i < _pdfLines.Length; i++)
            {
                ...
                if (!_readOrdenCompra)
                {
                    if (_pdfLines[i].Trim().Contains(" Pedido Prove F Revisión "))
                    {
                ...
                if (!_readRut)
                {
                    if (_pdfLines[i].Trim().Contains("Inscr IVA:"))
                    {
