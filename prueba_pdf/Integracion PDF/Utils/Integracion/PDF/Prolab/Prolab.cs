...
namespace IntegracionPDF.Integracion_PDF.Utils.Integracion.PDF.Prolab
{
    class Prolab
    {
        #region Variables
        ...
        public Prolab(PDFReader pdfReader)
        {
            ...
            //foreach(var str in pdfLines)
            {
                ...
                switch (optItem)
                {
                    ...
                    case 1:
                        ...
                        var previousLine = pdfLines[i - 1].Trim().DeleteContoniousWhiteSpace();
                        var nextLine = pdfLines[i + 1].Trim().DeleteContoniousWhiteSpace();
                        if (GetFormatItemsPattern(previousLine) == -1)
                        {
                            var split = previousLine.Split(' ');
                            item1.Cantidad = split[0];
                            item1.Precio = split[split.Length - 2];
                        }else if(GetFormatItemsPattern(nextLine) == -1)
                        {
