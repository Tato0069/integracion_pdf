...
namespace IntegracionPDF.Integracion_PDF.Utils.Integracion.PDF.Toc
{
    public class Toc
    {
        private const string RutDelimit = "R.U.T.:";
        private const string OrdenCompraDelimit = "ORDEN DE COMPRA";
        private const string ItemDelimit = "CODIGO COD.BARRA DESCRIPCION CANT. P.NETO P.TOTAL";
        ...
        private List<Item> GetItems(string[] pdfLines, int firstIndex)
        {
            ...
            {
                var str = pdfLines[firstIndex];
                //Es una linea de Items
                if (Regex.Match(str, @"\s[a-zA-Z]{1,2}\d{5,6}\s").Success)
                {
                    ...
                    var item = new Item
                    {
                        Sku = GetSku(pdfLines[firstIndex]),
        ...
        {
            var index = Regex.Match(str, @"\s[a-zA-Z]{1,2}\d{5,6}\s").Index;
            var length = Regex.Match(str, @"\s[a-zA-Z]{1,2}\d{5,6}\s").Length;
        ...
        public OrdenCompra.OrdenCompra GetOrdenCompra()
        {
            ...
            for (var i = 0; i < _pdfLines.Length; i++)
            {
                if (!_readRut)
                {
                    if (_pdfLines[i].Trim().Contains(RutDelimit))
                    {
                ...
                if (!_readOrdenCompra)
                {
                    if (_pdfLines[i].Trim().Contains(OrdenCompraDelimit))
                    {
